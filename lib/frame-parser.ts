...
const parseNodeIdentificationPayload = function(frame, reader) {
...
const ParseIOSamplePayload = function(frame, reader, options) {
...
// Series 1 Support
const received16BitPacketIO = function(frame, reader) {
  let hasDigital = 0;
  const data = {
...
frame_parser[C.FRAME_TYPE.AT_COMMAND_RESPONSE] = function(frame, reader) {
  ...
  if ((frame.command === "ND") && (frame.commandStatus === C.COMMAND_STATUS.OK) && (reader.buf.length > reader.tell())) {
    ...
    parseNodeIdentificationPayload(frame.nodeIdentification, reader);
  } else {
